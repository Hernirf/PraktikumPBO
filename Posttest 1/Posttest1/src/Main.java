    ...
    public static void readLoker() throws IOException{
        if (data_loker.size()==0){
            ...
            for (int i=0; i <  data_loker.size(); i++){
                System.out.println("Nama        : " + data_loker.get(i).nama);
                System.out.println("Nomor HP    : " + data_loker.get(i).nomorHP);
                System.out.println("Nomor Loker : " + data_loker.get(i).nomor);
                System.out.println("Nama Barang : " + data_loker.get(i).nama_barang);
                System.out.println("Tanggal     : " + data_loker.get(i).tanggal);
                System.out.println("\n");
            }
        }
    }
    ...
    public static void upLoker() throws IOException{
        ...
        for (int i=0; i <  data_loker.size(); i++){
            if (idx == data_loker.get(i).nomor){
                ...
                System.out.print("Masukkan tanggal: ");
                String tanggal = br.readLine();
                LokerData loker = new LokerData(idx, namaa, nama_ttp, nomorHPP, tanggal);
                data_loker.set(i,loker);
                kondisi = 1;
